...
export const placeOrder = async (req, res) => {
  ...
  if (!cart || cart.items.length === 0)
    return res
      .status(400)
      .json({ satus: "fail", data: null, message: "cart empty" });
...
export const getMyOrders = async (req, res) => {
  ...
  res.json({ status: "success", data: orders, message: "ordered fetched" });
};
export const getOrderById = async (req, res) => {
  ...
  if (!order)
    return res
      .status(404)
      .json({ satus: "fail", data: null, message: "not found" });
...
export const cancelOrder = async (req, res) => {
  ...
  if (!order || order.status !== "pending")
    return res
      .status(400)
      .json({ satus: "fail", data: null, message: "cannot cancel" });
